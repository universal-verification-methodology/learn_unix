# Module 04: Globs & wildcards

**Kind:** `lab` · Primary lab: `wildcards-globs` · **Shipped**

[← Absolute vs relative paths](../module03-path-abs-rel/README.md) · [Course README](../README.md) · [File types & links →](../module05-file-types-lab/README.md)

## Outcomes

After this module you can explain and practice the ideas taught by **`wildcards-globs`**, in the browser and/or on a real shell.

## Two tracks (pick one or both)

### Track A — Real Unix (hands-on)

Work in your local terminal using this module’s `examples/` (copied from the proven `learn_unix_git` trees).

1. Open [EXAMPLES.md](EXAMPLES.md) and run the “Try these” commands.
2. Complete [CHECKLIST.md](CHECKLIST.md) on a real filesystem (`~/unix_practice` recommended).
3. Optional self-check: `./scripts/module.sh 04 --check` (from course root).

### Track B — Browser lab (online)

1. Local: [http://127.0.0.1:8080/tools/wildcards-globs/index.html](http://127.0.0.1:8080/tools/wildcards-globs/index.html)
2. Live: [https://universal-verification-methodology.github.io/learning/tools/wildcards-globs/](https://universal-verification-methodology.github.io/learning/tools/wildcards-globs/)
3. Load the **starter example**, then work challenges.
4. Check off the Track B items in [CHECKLIST.md](CHECKLIST.md).

> Concept labs are literacy tools — they do not replace a real shell for scripts you will keep in a design project.

## Media (planned)

| Artifact | Path |
|----------|------|
| Outline | [outline.yaml](outline.yaml) |
| Transcript stub | [transcript.md](transcript.md) |
| Slides / video | `media/module04/` (generate later) |

## Files

```
module04-wildcards-globs/
├── README.md       # this file
├── CHECKLIST.md    # Track A + Track B checkboxes
├── EXAMPLES.md     # real-Unix example index
├── outline.yaml    # slide / clip outline
├── transcript.md   # speakable stub
└── examples/       # hands-on trees (Track A)
```
